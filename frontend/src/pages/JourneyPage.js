...
const JourneyPage = () => {
  ...
  useEffect(() => {
    if (isJourneyActive) {
      ...
      return () => {
        ...
        document.title = "HRMS Luminoid - Journey Tracking";
      };
    } else {
      document.title = "HRMS Luminoid - Journey Tracking";
    }
  ...
  const enableBackgroundTracking = async () => {
    if (!isBackgroundTrackingSupported()) {
      toast.error(
        "Background tracking is not supported on this device/browser",
      );
      return;
    }
    ...
    toast.success(
      "🎯 Background tracking enabled! Location will be tracked even when app is closed.",
    );
  };
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            ...
            // Send first GPS ping
            await api.post("/api/journey/ping", {
              lat,
              lng,
              accuracy,
              batteryLevel: 100,
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            ...
            toast.success(
              `🏁 Journey ended! Total distance: ${response.data.totalDistanceKm || 0} km`,
            );
  ...
  if (journeyData === null)
    return (
      <div className="text-center py-5">
        ...
        <h5 style={{ color: "#475569", fontWeight: 700 }}>
          Journey Tracking Not Enabled
        </h5>
        <p style={{ color: "#94a3b8", fontSize: "0.9rem" }}>
          Contact HR to enable field employee tracking for your account.
        </p>
      </div>
    );
    ...
    <div className="fade-in-up pb-5">
      ...
      <div className="page-header">
        <h1 className="page-title">
          <i className="fas fa-route me-2" style={{ color: "#10b981" }} />
          Journey Tracker
        </h1>
        <p className="text-muted">
          Track your daily travel distance in real-time
        </p>
      </div>
        ...
        <Card.Body style={{ padding: "1.75rem" }}>
          ...
          <div className="d-flex justify-content-between align-items-start mb-3">
            ...
            {isJourneyActive && (
              ...
              >
                <div
                  style={{ fontSize: "0.65rem", opacity: 0.8, marginBottom: 2 }}
                >
                  GPS TRACKING
                </div>
            ...
            >
              ...
              >
                ...
                <div style={{ textAlign: "left", paddingLeft: "1rem" }}>
                  ...
                  <br />❌ <strong>DO NOT close this browser tab</strong>
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                {journey?.locationPoints?.length || 0} points recorded
              </div>
            </div>
          )}
                ...
                {[
                  {
                    icon: "play-circle",
                    label: "Started",
                    value: fmt(journey.startTime),
                  },
                  {
                    icon: "stop-circle",
                    label: "Ended",
                    value: journey.endTime ? fmt(journey.endTime) : "Ongoing",
                  },
                  {
                    icon: "clock",
                    label: "Duration",
                    value: duration(journey.startTime, journey.endTime),
                  },
                  {
                    icon: "map-pin",
                    label: "GPS Points",
                    value: journey.locationPoints?.length || 0,
                  },
                ].map((s, i) => (
                  <div
                    key={i}
                ...
                {[
                  {
                    icon: "tachometer-alt",
                    label: "Avg Speed",
                    value: `${journey.avgSpeedKmh || 0} km/h`,
                    color: "rgba(59,130,246,0.15)",
                  },
                  {
                    icon: "rocket",
                    label: "Max Speed",
                    value: `${journey.maxSpeedKmh || 0} km/h`,
                    color: "rgba(239,68,68,0.15)",
                  },
                  {
                    icon: "pause-circle",
                    label: "Paused",
                    value: `${journey.totalPausedMinutes || 0} min`,
            ...
            {!hasCheckedIn && !journey && (
              ...
              >
                ...
                <span style={{ fontSize: "0.9rem", opacity: 0.85 }}>
                  Please check in attendance first to start your journey
                </span>
              </div>
            )}
            {canStart && (
            ...
            {(journey?.status === "COMPLETED" ||
              ...
              >
                ...
                <span style={{ fontSize: "0.9rem" }}>
                  Journey completed for today · {journey.totalDistanceKm} km
                </span>
              </div>
            )}
          </div>
              ...
              {[
                {
                  ...
                  desc: "Mark your attendance first from the Attendance page",
                },
                {
                  step: "2",
                  icon: "play-circle",
                  color: "#10b981",
                  title: "Start Journey",
                  desc: "Tap Start Journey — GPS tracking begins automatically",
                },
                {
                  step: "3",
                  icon: "satellite-dish",
                  color: "#8b5cf6",
                  title: "Travel & Track",
                  desc: "GPS pings every 60 seconds, distance calculated automatically",
                },
                {
                  step: "4",
                  icon: "stop-circle",
                  color: "#ef4444",
                  title: "End Journey",
                  desc: "Tap End Journey when done — total KM saved to your record",
                },
              ].map((s) => (
      ...
      {isJourneyActive && journey?.locationPoints?.length > 0 && (
        ...
        >
          <Card.Body style={{ padding: "1.25rem" }}>
            ...
            >
              <i className="fas fa-map-pin me-2" style={{ color: "#10b981" }} />
              Live GPS Points ({journey.locationPoints.length} recorded)
            </div>
                ...
                .map((pt, i) => (
                  ...
                  >
                    ...
                    <div className="d-flex gap-3" style={{ color: "#6b7280" }}>
                      {pt.distanceFromLast > 0 && (
                        <span>+{(pt.distanceFromLast * 1000).toFixed(0)}m</span>
                      )}
      ...
      >
        <Card.Body style={{ padding: "1.25rem" }}>
          ...
          >
            <i className="fas fa-history me-2" style={{ color: "#10b981" }} />
            Journey History
          </div>
                ...
                {[
                  {
                    ...
                    value: journeyHistory.length,
                    icon: "route",
                    color: "#10b981",
                  },
                  {
                    label: "Total Distance",
                    value: `${journeyHistory.reduce((s, j) => s + (j.totalDistanceKm || 0), 0).toFixed(1)} km`,
                    icon: "road",
                    color: "#10b981",
                  },
                  {
                    label: "Avg per Day",
                    value: `${(journeyHistory.reduce((s, j) => s + (j.totalDistanceKm || 0), 0) / journeyHistory.length).toFixed(1)} km`,
                    icon: "chart-line",
                    color: "#10b981",
                  },
                  {
                    label: "This Month",
                    value: `${journeyHistory
                      .filter(
                        (j) =>
                          new Date(j.date).getMonth() === new Date().getMonth(),
                      )
                      .reduce((s, j) => s + (j.totalDistanceKm || 0), 0)
                      .toFixed(1)} km`,
                    icon: "calendar",
                    color: "#10b981",
                  },
                ...
                >
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      ...
                      >
                        {fmt(j.startTime)} →{" "}
                        {j.endTime ? fmt(j.endTime) : "Auto-ended"} ·{" "}
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            <i className="fas fa-map me-2" />
            Today's Journey Route
          </Modal.Title>
        </Modal.Header>
        <Modal.Body style={{ padding: 0 }}>
          {journey?.locationPoints?.length > 0 &&
            (() => {
              const positions = journey.locationPoints.map((p) => [
                p.lat,
                p.lng,
              ]);
              return (
      ...
      >
        ...
        <Modal.Body style={{ padding: "2rem" }}>
          <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
            ...
            <p
              style={{ color: "#64748b", fontSize: "0.9rem", marginBottom: 0 }}
            >
              Continue tracking your journey even when the app is closed or
              minimized
            </p>
          </div>
          ...
          >
            <div style={{ fontSize: "0.8rem", color: "#92400e" }}>
              ...
              <strong>Note:</strong> You'll need to allow notifications and
              location permissions for background tracking to work.
            </div>
          </div>
