...
exports.startJourney = async (req, res) => {
  try {
    ...
    if (!user.isFieldEmployee) {
      return res.status(403).json({ message: 'Only field employees can start a journey' });
    }
    ...
    if (!attendance?.checkIn) {
      return res.status(400).json({ message: 'Please check in attendance first before starting journey' });
    }

    // Check no active journey today
    const existing = await Journey.findOne({ employeeId: userId, date: { $gte: todayStart, $lte: todayEnd } });
    if (existing) {
      return res.status(400).json({ message: existing.status === 'ACTIVE' ? 'Journey already active' : 'Journey already completed today' });
    }
    ...
    const journey = await Journey.create({
      ...
      startTime: new Date(),
      status: 'ACTIVE',
      startLocation: lat && lng ? { lat, lng, accuracy, timestamp: new Date() } : null,
      locationPoints: lat && lng ? [{ lat, lng, accuracy, distanceFromLast: 0, speedKmh: 0, timestamp: new Date() }] : []
    });

    res.status(201).json({ message: 'Journey started with location captured', journey });
  } catch (error) {
...
exports.pingLocation = async (req, res) => {
  try {
    ...
    const journey = await Journey.findOne({ employeeId: userId, date: { $gte: todayStart, $lte: todayEnd }, status: 'ACTIVE' });
    if (!journey) return res.status(404).json({ message: 'No active journey found' });

    // Filter: bad GPS accuracy (> 100m)
    if (accuracy && accuracy > 100) {
      return res.json({ message: 'Point skipped: low GPS accuracy', totalDistanceKm: journey.totalDistanceKm });
    }
    ...
    if (journey.locationPoints.length > 0) {
      const lastPoint = journey.locationPoints[journey.locationPoints.length - 1];
      distanceFromLast = haversine(lastPoint, { lat, lng });

      // Calculate speed (km/h)
      const timeDiffSeconds = (new Date() - new Date(lastPoint.timestamp)) / 1000;
      ...
      if (distanceFromLast < 0.05) {
        ...
        journey.locationPoints.push({ lat, lng, accuracy, distanceFromLast: 0, speedKmh: 0, batteryLevel, timestamp: new Date() });
        await journey.save();
        return res.json({ message: 'Point saved: no significant movement', totalDistanceKm: journey.totalDistanceKm });
      }

      // Filter: GPS jump > 5km in one ping (spoof/error)
      if (distanceFromLast > 5) {
        return res.json({ message: 'Point skipped: GPS anomaly detected', totalDistanceKm: journey.totalDistanceKm });
      }
    ...
    if (batteryLevel !== undefined) {
      journey.batteryLevels.push({ level: batteryLevel, timestamp: new Date() });
    ...
    journey.locationPoints.push({ lat, lng, accuracy, distanceFromLast, speedKmh, batteryLevel, timestamp: new Date() });
    journey.totalDistanceKm = Math.round((journey.totalDistanceKm + distanceFromLast) * 100) / 100;
    
    // Calculate average speed
    const totalPoints = journey.locationPoints.length;
    if (totalPoints > 1) {
      const totalSpeed = journey.locationPoints.reduce((sum, p) => sum + (p.speedKmh || 0), 0);
...
exports.pauseJourney = async (req, res) => {
  try {
    ...
    const journey = await Journey.findOne({ employeeId: userId, date: { $gte: todayStart, $lte: todayEnd }, status: 'ACTIVE' });
    if (!journey) return res.status(404).json({ message: 'No active journey to pause' });

    journey.status = 'PAUSED';
    journey.pausedAt = new Date();
    journey.pauseHistory.push({ pausedAt: new Date(), reason: reason || 'Manual pause' });
    await journey.save();
...
exports.resumeJourney = async (req, res) => {
  try {
    ...
    const journey = await Journey.findOne({ employeeId: userId, date: { $gte: todayStart, $lte: todayEnd }, status: 'PAUSED' });
    if (!journey) return res.status(404).json({ message: 'No paused journey to resume' });

    const pauseDuration = Math.round((new Date() - new Date(journey.pausedAt)) / 60000); // minutes
    journey.totalPausedMinutes = (journey.totalPausedMinutes || 0) + pauseDuration;
    
    // Update last pause history entry
    if (journey.pauseHistory.length > 0) {
      journey.pauseHistory[journey.pauseHistory.length - 1].resumedAt = new Date();
    }
...
exports.endJourney = async (req, res) => {
  try {
    ...
    const journey = await Journey.findOne({ employeeId: userId, date: { $gte: todayStart, $lte: todayEnd }, status: { $in: ['ACTIVE', 'PAUSED'] } });
    if (!journey) return res.status(404).json({ message: 'No active journey to end' });

    // If paused, add final pause duration
    if (journey.status === 'PAUSED' && journey.pausedAt) {
      const pauseDuration = Math.round((new Date() - new Date(journey.pausedAt)) / 60000);
      journey.totalPausedMinutes = (journey.totalPausedMinutes || 0) + pauseDuration;
    }
    ...
    if (lat && lng) {
      ...
      journey.locationPoints.push({ lat, lng, accuracy, distanceFromLast: 0, speedKmh: 0, timestamp: new Date() });
      
      // Calculate total distance from start to end
      if (journey.startLocation && journey.startLocation.lat && journey.startLocation.lng) {
    ...
    res.json({ message: 'Journey ended', journey, totalDistanceKm: journey.totalDistanceKm });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.getTodayJourney = async (req, res) => {
  try {
    ...
    const journey = await Journey.findOne({ employeeId: req.user.id, date: { $gte: todayStart, $lte: todayEnd } });
...
exports.getActiveJourneys = async (req, res) => {
  try {
    ...
    // Get last location for each journey
    const journeysWithLocation = activeJourneys.map(j => {
      const lastPoint = j.locationPoints[j.locationPoints.length - 1];
      return {
        ...j.toObject(),
        lastLocation: lastPoint || null,
        lastUpdateMinutesAgo: lastPoint ? Math.round((new Date() - new Date(lastPoint.timestamp)) / 60000) : null
      };
    });
...
exports.getJourneyAnalytics = async (req, res) => {
  try {
    ...
    const start = startDate ? moment.tz(startDate, 'Asia/Kolkata').startOf('day').toDate() : moment.tz('Asia/Kolkata').subtract(30, 'days').startOf('day').toDate();
    const end = endDate ? moment.tz(endDate, 'Asia/Kolkata').endOf('day').toDate() : moment.tz('Asia/Kolkata').endOf('day').toDate();
    ...
    const journeys = await Journey.find(filter).populate('employeeId', 'firstName lastName department');
    ...
    const totalDistance = filteredJourneys.reduce((sum, j) => sum + (j.totalDistanceKm || 0), 0);
    const avgDistance = totalJourneys > 0 ? totalDistance / totalJourneys : 0;
    const maxDistance = filteredJourneys.reduce((max, j) => Math.max(max, j.totalDistanceKm || 0), 0);
    const totalEmployees = new Set(filteredJourneys.map(j => j.employeeId?._id?.toString())).size;
...
exports.getJourneyHistory = async (req, res) => {
  try {
    ...
    if (startDate || endDate) {
      filter.date = {};
      if (startDate) filter.date.$gte = moment.tz(startDate, 'Asia/Kolkata').startOf('day').toDate();
      if (endDate) filter.date.$lte = moment.tz(endDate, 'Asia/Kolkata').endOf('day').toDate();
    }
