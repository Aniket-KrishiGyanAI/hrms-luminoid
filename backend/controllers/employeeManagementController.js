...
const deactivateEmployee = async (req, res) => {
  try {
    ...
    const { exitReason, exitDate, exitInterview, handoverStatus, exitNotes } =
      req.body;
    ...
    res
      .status(500)
      .json({ message: "Error deactivating employee", error: error.message });
  }
};
...
const reactivateEmployee = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error reactivating employee", error: error.message });
  }
};
...
const getAllEmployees = async (req, res) => {
  try {
    ...
    const employees = await User.find(filter)
      .select(
        "firstName lastName email role department designation joinDate isActive profileImage isFieldEmployee",
      )
    ...
    const deptMap = Object.fromEntries(
      depts.map((d) => [d._id.toString(), d.name]),
    );
...
const toggleFieldEmployee = async (req, res) => {
  try {
    ...
    await user.save();
    res.json({
      message: `Field employee ${user.isFieldEmployee ? "enabled" : "disabled"}`,
